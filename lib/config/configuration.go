...
type CommandLineFlags struct {
	...
	// configuration. Note, this makes sure Teleport starts with the appropriate
	// configuration, however the binary still has to be compiled against
	// dev.boringcrypto.
	FIPS bool
}
...
func Configure(clf *CommandLineFlags, cfg *service.Config) error {
	...
	// If FIPS mode is specified, validate Teleport configuration is FedRAMP compliant.
	if clf.FIPS {
		...
		// Make sure cluster settings are also FedRAMP compliant.
		if cfg.Auth.Enabled {
			// Only SSO based authentication is supported. The SSO provider is where
			// any FedRAMP compliance (like password complexity) should be enforced.
			//
